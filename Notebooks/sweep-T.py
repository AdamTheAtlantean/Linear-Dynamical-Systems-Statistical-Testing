...
from var_model import fit_var_and_components, unpack_B_to_Phi
...
1. T values in run_T_sweep(...)
2. repeat index r in run_T_sweep_with_variance(...)
...
def _run_single_T_value(
    ...
    print("\n" + "=" * 70)
    print(f"Running isotropic T-sweep experiment for T = {T}  (n = {n}, p = {p})")
    print("=" * 70)
...
def _run_single_repeat_T(
    r: int,
    T_values,
    **kwargs
):
    print(f"\n=== Repeat {r+1} ===")
    return r, run_T_sweep(
        T_values=T_values,
        seed=r,
        **kwargs
    )
...
def run_T_sweep(
    ...
    """
    Sweep over effective sample size T where n = T + p.

    Parallel over T values only.
    ...
    sweep_results.sort(key=lambda row: T_values.index(row["T"]))
    return sweep_results


def run_T_sweep_with_variance(
    T_values,
    n_repeats=100,
    n_jobs: int = 1,
    verbose_joblib: int = 0,
    **kwargs
    ...
    all_results = {T: {"auc": [], "f1": [], "best_threshold": []} for T in T_values}
    ...
    for _, results in repeat_outputs:
        for row in results:
            T = row["T"]
            all_results[T]["auc"].append(row["auc"])
            all_results[T]["f1"].append(row["f1"])
            all_results[T]["best_threshold"].append(row["best_threshold"])

    summary = []

    for T in T_values:
        auc_arr = np.array(all_results[T]["auc"])
        f1_arr  = np.array(all_results[T]["f1"])
        thresh_arr = np.array(all_results[T]["best_threshold"])
...
def plot_T_sweep_results(sweep_results):
    ...
    plt.xlabel("Effective sample size T")
    plt.ylabel("AUC")
    plt.title("AUC vs T (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()
    ...
    plt.xlabel("Effective sample size T")
    plt.ylabel("Score")
    plt.title("Classification metrics vs T (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.show()
    ...
    plt.xlabel("Effective sample size T")
    plt.ylabel("Rate")
    plt.title("Error Rates vs T (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.show()
    ...
    plt.title("Best threshold vs T (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()


def plot_T_with_variance(summary):
    T_vals = np.array([r["T"] for r in summary])
    ...
    plt.figure(figsize=(7, 4))
    plt.errorbar(T_vals, auc_mean, yerr=auc_std, marker="o", capsize=4)
    plt.xlabel("Effective sample size T")
    plt.ylabel("AUC")
    plt.title("AUC vs T (mean ± std) — Isotropic Metric, F-band")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(7, 4))
    plt.errorbar(T_vals, f1_mean, yerr=f1_std, marker="o", capsize=4)
    plt.xlabel("Effective sample size T")
    plt.ylabel("F1")
    plt.title("F1 vs T (mean ± std) — Isotropic Metric, F-band")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(7, 4))
    plt.errorbar(T_vals, thresh_mean, yerr=thresh_std, marker="o", capsize=4)
    plt.xlabel("Effective sample size T")
    plt.ylabel("Best threshold")
    plt.title("Best threshold vs T (mean ± std) — Isotropic Metric, F-band")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    T_values = [100, 200, 400, 600, 800, 1000, 1200, 1500, 1800]
    ...
    if USE_VARIANCE_SWEEP:
        summary = run_T_sweep_with_variance(
            T_values=T_values,
            n_repeats=100,
        ...
        print("\n" + "=" * 90)
        print("ISOTROPIC T-SWEEP SUMMARY TABLE (100 REPEATS, F-BAND)")
        print("=" * 90)
        for row in summary:
            print(
                f"T={row['T']:>4d} | "
        ...
        print("\n" + "=" * 90)
        print("ISOTROPIC T-SWEEP SUMMARY TABLE (SINGLE RUN, F-BAND)")
        print("=" * 90)
