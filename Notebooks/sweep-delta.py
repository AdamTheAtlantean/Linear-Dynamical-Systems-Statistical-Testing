...
1. delta values in run_regime_sweep(...)
2. repeat index r in run_regime_sweep_with_variance(...)
...
def regimes_from_delta(a: float, w: float, delta: float):
    """
    Build the two DIFFERENT rho(F)-regime intervals from a width w and gap delta.
...
def validate_regime_bounds(
    ...
    """
    Ensure all generated right-hand rho(F)-regimes stay below rho_upper_limit.
    """
...
def _run_single_delta_value(
    ...
    print("\n" + "=" * 70)
    print(
        f"Running rho(F) regime-sweep experiment for delta = {delta:.4f} | "
...
def _run_single_repeat_regime(
    r: int,
    delta_values,
    **kwargs
):
    print(f"\n=== Repeat {r+1} ===")
    return r, run_regime_sweep(
...
def run_regime_sweep(
    ...
    DIFFERENT regimes:
        y3: [a, a + w]
        y4: [a + w + delta, a + 2w + delta]

    All of these are rho(F)-regimes.
    """
    ...
    with parallel_config(backend="loky", inner_max_num_threads=1):
        sweep_results = Parallel(n_jobs=n_jobs, verbose=verbose_joblib)(
            delayed(_run_single_delta_value)(
                delta=delta,
                regime_name=regime_name,
                rho_min=rho_min,
                rho_max=rho_max,
    ...
    sweep_results.sort(key=lambda row: delta_values.index(row["delta"]))
    return sweep_results



# Repeated regime sweep


def run_regime_sweep_with_variance(
    delta_values,
    n_repeats=100,
    n_jobs: int = 1,
    verbose_joblib: int = 0,
    **kwargs
    ...
    all_results = {
        float(delta): {"auc": [], "f1": [], "best_threshold": []}
        for delta in delta_values
    }
    ...
    if n_jobs == 1:
        ...
        with parallel_config(backend="loky", inner_max_num_threads=1):
            repeat_outputs = Parallel(n_jobs=n_jobs, verbose=verbose_joblib)(
                delayed(_run_single_repeat_regime)(
                    r=r,
                    delta_values=delta_values,
                    n_jobs=1,
                    **kwargs
    ...
    for _, results in repeat_outputs:
        for row in results:
            ...
            all_results[delta]["auc"].append(row["auc"])
            all_results[delta]["f1"].append(row["f1"])
            all_results[delta]["best_threshold"].append(row["best_threshold"])

    summary = []

    for delta in delta_values:
        delta = float(delta)
        auc_arr = np.array(all_results[delta]["auc"])
        f1_arr = np.array(all_results[delta]["f1"])
        thresh_arr = np.array(all_results[delta]["best_threshold"])
...
def plot_regime_sweep_results(sweep_results):
    ...
    plt.ylabel("AUC")
    plt.title(r"AUC vs rho(F)-regime gap $\delta$ (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()
    ...
    plt.xlabel(r"Gap $\delta$")
    plt.ylabel("Score")
    plt.title(r"Classification metrics vs rho(F)-regime gap $\delta$ (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.show()
    ...
    plt.xlabel(r"Gap $\delta$")
    plt.ylabel("Rate")
    plt.title(r"Error rates vs rho(F)-regime gap $\delta$ (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.show()
    ...
    plt.title(r"Best threshold vs rho(F)-regime gap $\delta$ (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()


def plot_regime_with_variance(summary):
    delta_vals = np.array([r["delta"] for r in summary])
    ...
    plt.figure(figsize=(7, 4))
    plt.errorbar(delta_vals, auc_mean, yerr=auc_std, marker="o", capsize=4)
    plt.xlabel(r"Gap $\delta$")
    plt.ylabel("AUC")
    plt.title(r"AUC vs rho(F)-regime gap $\delta$ (mean ± std) — Isotropic Metric, F-band")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(7, 4))
    plt.errorbar(delta_vals, f1_mean, yerr=f1_std, marker="o", capsize=4)
    plt.xlabel(r"Gap $\delta$")
    plt.ylabel("F1")
    plt.title(r"F1 vs rho(F)-regime gap $\delta$ (mean ± std) — Isotropic Metric, F-band")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(7, 4))
    plt.errorbar(delta_vals, thresh_mean, yerr=thresh_std, marker="o", capsize=4)
    plt.xlabel(r"Gap $\delta$")
    plt.ylabel("Best threshold")
    plt.title(r"Best threshold vs rho(F)-regime gap $\delta$ (mean ± std) — Isotropic Metric, F-band")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    if USE_VARIANCE_SWEEP:
        summary = run_regime_sweep_with_variance(
            delta_values=delta_values,
            n_repeats=100,
        ...
        print("\n" + "=" * 100)
        print("ISOTROPIC RHO(F) REGIME-SWEEP SUMMARY TABLE (100 REPEATS, F-BAND)")
        print("=" * 100)
        for row in summary:
            print(
        ...
        plot_regime_with_variance(summary)
        ...
        print("\n" + "=" * 100)
        print("ISOTROPIC RHO(F) REGIME-SWEEP SUMMARY TABLE (SINGLE RUN, F-BAND)")
        print("=" * 100)
