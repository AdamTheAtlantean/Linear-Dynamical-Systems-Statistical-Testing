...
1. d_y values in run_dy_sweep(...)
2. repeat index r in run_dy_sweep_with_variance(...)
...
def run_realization_sensitivity(
    ...
    print(f"\n--- Regime: {regime_name}  rho(F) in [{rho_min}, {rho_max}] | d_y = {d_y} ---\n")
...
def _run_single_dy_value(
    ...
    print("\n" + "=" * 70)
    print(f"Running isotropic d_y-sweep experiment for d_y = {d_y}")
    print("=" * 70)
...
def _run_single_repeat_dy(
    r: int,
    dy_values,
    **kwargs
):
    print(f"\n=== Repeat {r+1} ===")
    return r, run_dy_sweep(
        dy_values=dy_values,
        seed=r,
        **kwargs
    )
...
def run_dy_sweep(
    ...
    """
    Sweep over observation dimension d_y.

    Parallel over d_y values only.

    Regimes are defined by rho(F), where F = A - L C.
    """
    if n_jobs == 1:
        sweep_results = []
        for d_y in dy_values:
            row = _run_single_dy_value(
                d_y=d_y,
                regime_name=regime_name,
                rho_min=rho_min,
                rho_max=rho_max,
    ...
    sweep_results.sort(key=lambda row: dy_values.index(row["d_y"]))
    return sweep_results


def run_dy_sweep_with_variance(
    dy_values,
    n_repeats=100,
    n_jobs: int = 1,
    verbose_joblib: int = 0,
    **kwargs
    ...
    all_results = {d_y: {"auc": [], "f1": [], "best_threshold": []} for d_y in dy_values}

    if n_jobs == 1:
        repeat_outputs = []
        for r in range(n_repeats):
            print(f"\n=== Repeat {r+1}/{n_repeats} ===")
            results = run_dy_sweep(
                dy_values=dy_values,
                seed=r,
                n_jobs=1,
                **kwargs
    ...
    for _, results in repeat_outputs:
        for row in results:
            d_y = row["d_y"]
            all_results[d_y]["auc"].append(row["auc"])
            all_results[d_y]["f1"].append(row["f1"])
            all_results[d_y]["best_threshold"].append(row["best_threshold"])

    summary = []

    for d_y in dy_values:
        auc_arr = np.array(all_results[d_y]["auc"])
        f1_arr  = np.array(all_results[d_y]["f1"])
        thresh_arr = np.array(all_results[d_y]["best_threshold"])
...
def plot_dy_sweep_results(sweep_results):
    dy_vals = np.array([r["d_y"] for r in sweep_results], dtype=int)
    ...
    plt.plot(dy_vals, auc_vals, marker="o", linewidth=2)
    plt.xlabel(r"Observation dimension $d_y$")
    plt.ylabel("AUC")
    plt.title(r"AUC vs $d_y$ (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(7, 4))
    plt.plot(dy_vals, f1_vals, marker="o", linewidth=2, label="F1")
    plt.plot(dy_vals, tpr_vals, marker="o", linewidth=2, label="TPR")
    plt.plot(dy_vals, 1 - fpr_vals, marker="o", linewidth=2, label="TNR")
    plt.xlabel(r"Observation dimension $d_y$")
    plt.ylabel("Score")
    plt.title(r"Classification metrics vs $d_y$ (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.show()
    ...
    plt.figure(figsize=(7, 4))
    plt.plot(dy_vals, fpr_vals, marker="o", linewidth=2, label="FPR")
    plt.plot(dy_vals, fnr_vals, marker="o", linewidth=2, label="FNR")
    plt.plot(dy_vals, tpr_vals, marker="o", linewidth=2, linestyle="--", label="TPR")
    plt.xlabel(r"Observation dimension $d_y$")
    plt.ylabel("Rate")
    plt.title(r"Error rates vs $d_y$ (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.show()
    ...
    plt.figure(figsize=(7, 4))
    plt.plot(dy_vals, thresh_vals, marker="o", linewidth=2)
    plt.xlabel(r"Observation dimension $d_y$")
    plt.ylabel("Best threshold")
    plt.title(r"Best threshold vs $d_y$ (Isotropic Metric, F-band)")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()


def plot_dy_with_variance(summary):
    dy_vals = np.array([r["d_y"] for r in summary])
    ...
    plt.figure(figsize=(7, 4))
    plt.errorbar(dy_vals, auc_mean, yerr=auc_std, marker="o", capsize=4)
    plt.xlabel(r"Observation dimension $d_y$")
    plt.ylabel("AUC")
    plt.title(r"AUC vs $d_y$ (mean ± std) — Isotropic Metric, F-band")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(7, 4))
    plt.errorbar(dy_vals, f1_mean, yerr=f1_std, marker="o", capsize=4)
    plt.xlabel(r"Observation dimension $d_y$")
    plt.ylabel("F1")
    plt.title(r"F1 vs $d_y$ (mean ± std) — Isotropic Metric, F-band")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(7, 4))
    plt.errorbar(dy_vals, thresh_mean, yerr=thresh_std, marker="o", capsize=4)
    plt.xlabel(r"Observation dimension $d_y$")
    plt.ylabel("Best threshold")
    plt.title(r"Best threshold vs $d_y$ (mean ± std) — Isotropic Metric, F-band")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    if USE_VARIANCE_SWEEP:
        summary = run_dy_sweep_with_variance(
            dy_values=dy_values,
            n_repeats=100,
        ...
        print("\n" + "=" * 90)
        print("ISOTROPIC d_y-SWEEP SUMMARY TABLE (100 REPEATS, F-BAND)")
        print("=" * 90)
        for row in summary:
            print(
                f"d_y={row['d_y']:>2d} | "
        ...
        plot_dy_with_variance(summary)

    else:
        sweep_results = run_dy_sweep(
            dy_values=dy_values,
            seed=0,
        ...
        print("\n" + "=" * 90)
        print("ISOTROPIC d_y-SWEEP SUMMARY TABLE (SINGLE RUN, F-BAND)")
        print("=" * 90)
        ...
        plot_dy_sweep_results(sweep_results)